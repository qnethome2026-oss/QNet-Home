# Arduino ASR/TTS runtime contract

This contract replaces the earlier proposed HTTP `/listen` and `/speak`
...
| Application container | `qhome-voice-node-main-1` |
| Audio runner container | `qhome-voice-node-audio-analytics-runner-1` |
...
The contract is tied to these observed versions. An Arduino App CLI or App
...
device's `qhome-config.json` and restart the App; no model copy is required.
