...
if out=$(ssh -o ConnectTimeout=8 iq9 'systemctl is-active mosquitto geniex-serve qnet-agent | tr "\n" " "' 2>/dev/null); then
...
if line=$(ssh -o ConnectTimeout=8 iq9 "journalctl -u qnet-agent --no-pager | grep connected | tail -1" 2>/dev/null); then
...
if out=$(ssh -o ConnectTimeout=8 ventuno 'systemctl is-active qnet-vision qnet-look qnet-stream | tr "\n" " "' 2>/dev/null); then
...
ssh -o ConnectTimeout=8 ventuno 'docker ps --format "{{.Names}}" | grep -q qnet-voice-node-main' 2>/dev/null \
...
if out=$(ssh -o ConnectTimeout=8 ventuno2 'systemctl is-active qnet-look qnet-stream | tr "\n" " "' 2>/dev/null); then
...
ssh -o ConnectTimeout=8 ventuno2 'curl -s -m 20 http://127.0.0.1:9001/v1/models | grep -q qwen' 2>/dev/null \
...
ssh -o ConnectTimeout=8 ventuno2 'docker ps --format "{{.Names}}" | grep -q qnet-voice-node-main' 2>/dev/null \
